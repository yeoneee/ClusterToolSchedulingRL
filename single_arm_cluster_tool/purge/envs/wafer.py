...
class Wafer:
    status_dict = {
        'queue': -1,            # tool queue에 등록된 상태
        'inloadport': 0,        # loadport에 있는 상태
        'process': 1,           # tool에서 공정 진행 상태
        'finish': 2,            # tool에서 공정 완료 상태
        'outloadport': 3,       # loadport로 반출된 상태
        'exit': 4,               # loadport에서 반출된 상태
        'robot': 5
    }
    ...
    residency_time:torch.Tensor = None   # (batch, num_foup, foup_size, ll+stage num, 2) 해당 wafer가 해당 stage에 머무른 시간
