...
class LotStageEmbedding(nn.Module):
    def __init__(self, **params):
        ...
        self.col_cnt = len(params['group1_stage']) + len(params['group2_stage'])
    ...
    def forward(self, state):
        ...
        # feat 2
        stage_capacity = (state.stage.clone() / max(state.stage[0])
                          )[:, :, None].repeat(1, 1, 1).to(self.device)

        # feat 3
        stage_purge_time = torch.zeros_like(stage_capacity) # zero purge time
        ...
        # mat feature
        # ------------------------------------------------------
        flow = state.recipe_flow[:, :, 1:-1, None]
        norm_scale = 300
        process_time = state.recipe_time[:, :, 1:-1, None] / norm_scale # normalize
        cost_mat = torch.cat([flow,process_time],dim=-1)
        # shape: (batch, row, col, 2)
