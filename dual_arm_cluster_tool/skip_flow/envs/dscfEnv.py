...
class dscfEnv:
    """
    # dual-armed cluster tool for (shared) concurrent flow
    """
    ...
    def step(self, action: int) -> np.array:
        num_ll_unload_action = 2
        num_pm_unload_action = len(self.pms)
        num_load_action = len(self.pms) + 1 # PM + outLL
    ...
    def _get_action_mask(self) -> np.array:
        """
        ...
        def _can_unload_from_loadlock(loadlock, group):
            ...
            group_stage = self.group1_stage if group == 0 else self.group2_stage
            next_stage = next((i for i, s in enumerate(group_stage[0:], start=0+1) if s == 1), self.outloadlock)
        ...
        def _can_unload_from_pm(pm):
            ...
            if has_wafers_in_pm:
                next_stage = self._get_next_stage_of_wafer(pm.hold_wafer)
    ...
    def _get_next_stage_of_wafer(self, wafer: Wafer):
        group_stage = self.group1_stage if wafer.group == 0 else self.group2_stage
        next_stage = next((i for i, s in enumerate(group_stage[wafer.unloaded_stage:], start=wafer.unloaded_stage+1) if s == 1), self.outloadlock)

        return next_stage

    def _get_next_stage_of_pm(self, pm: Pm):
        group_stage = self.group1_stage if pm.hold_wafer.group == 0 else self.group2_stage
        next_stage = next((i for i, s in enumerate(group_stage[pm.stage:], start=pm.stage+1) if s == 1), self.outloadlock)
...
def main():
    ...
    policy = AlternatingSwapSequence(env)
    while not env.done:
        ...
        if action == 0:
            ...
            load_loc_id = action - num_ll_unload_action - num_pm_unload_action + 1
            if load_loc_id == len(env.pms) + 1:
                print(f"{i}th, Selected Action: Load to Outloadlock with Arm 1")
            else:
                print(f"{i}th, Selected Action: Load to PM {load_loc_id} with Arm 1")
        else:
            load_loc_id = action - num_ll_unload_action - num_pm_unload_action - num_load_action + 1
            if load_loc_id == len(env.pms) + 1:
                print(f"{i}th, Selected Action: Load to Outloadlock with Arm 2")
            else:
                print(f"{i}th, Selected Action: Load to PM {load_loc_id} with Arm 2")
        #env.show_state()
        print(f"Policy n: {policy.n}, Current Group: {policy.curr_group}, Current Plan: {policy.curr_cycle}, load: {policy.swap}")
        print("----")
