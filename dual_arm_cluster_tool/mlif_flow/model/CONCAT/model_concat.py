...
class Decoder(nn.Module):
    ...
    def get_single_arm_action_duration_embedding(self, state, env):
        ...
        if env.purge_constraint:
            # next stage's min purge time should be considered.
            # If there is an idle PM that can be loaded immediately in the next stage
            # earliest load start time == earlist arrival time
            # If not, you have to wait until the min purge time of the next stage.
            for loc in range(env.loc.num_loc):
                ...
                next_stage_loc_load_delay_time[next_stage_idle_loc] = 0 # idle PM
